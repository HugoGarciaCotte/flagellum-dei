...
const HostGame = () => {
  ...
  useEffect(() => {
    ...
    const refreshPlayerChar = async (characterId: string) => {
      // Feats now live on characters.feats — pulling the character row is enough.
  ...
  // Realtime: character updates by any player in this game.
  // Feats + subfeats now live inside characters.feats, so a single subscription is enough.
  useEffect(() => {
    if (!gameId) return;
